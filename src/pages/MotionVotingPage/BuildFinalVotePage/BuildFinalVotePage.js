...
export default function BuildFinalVotePage (props) {
  ...
  return (
    <div className="motion-creation-page">
      ...
      <div className="VotePage-view">
        ...
        <button
            ...
            onClick={() => {motionOwnerID == userId ? 
              ...
              :
              (axios.put("http://localhost:5000/motionuser", {"motionId" : motionID, "userId" : userId},{ 
              headers: {"Authorization" : `Bearer ${localStorage.getItem("token")}`, "Access-Control-Allow-Origin" : "*",
              "Access-Control-Allow-Methods" : "GET,PUT,POST,DELETE,PATCH,OPTIONS"}}))
              ;
              if(motionStatus) {
      ...
      {motionOwnerID == userId ? <button className = "skipSuggestionBtn" onClick = {() => axios.put(`http://localhost:5000/motions/${motionID}`, { 
              headers: {"Authorization" : `Bearer ${localStorage.getItem("token")}`, "Access-Control-Allow-Origin" : "*",
              "Access-Control-Allow-Methods" : "GET,PUT,POST,DELETE,PATCH,OPTIONS"}})}>Close Motion</button> : null};
    </div>
  );
}